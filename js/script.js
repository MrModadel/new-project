...
let arr = document.querySelectorAll('.promo__interactive-list li');
let u = document.querySelector('.promo__interactive-list');
...
for (let i of movies) {
   ...
   news.classList.add('promo__interactive-item');
   let d = document.createElement('div');
   d.classList.add('delete');
   news.innerText = i.Title;
   news.append(d);
   u.append(news);
   news.onclick=()=>{
      img.style.background = `url(${i.Poster}) center center/cover no-repeat`
   }
}
let arrOne = document.querySelectorAll('.promo__interactive-list li');

arrOne.forEach(item => {
   let d = item.querySelector('div');
   d.addEventListener('click', function () {
      let p = confirm('удалить?');
      if (p) {
         item.remove()
      }
   })
})
let left = document.querySelectorAll('.promo__menu-list li');
let block = document.querySelector('.promo__menu-list ul');
left.forEach(item => {
   item.remove();
})
let d = []
...
function onClickGenre() {
   let elems = document.querySelectorAll('.promo__menu-list a');
   elems.forEach(el => {
      el.onclick = () => {
         elems.forEach(el => el.classList.remove('promo__menu-item_active'));
